...
const Dashboard = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const reportRes = await axios.get(`${API_BASE_URL}/admin/monthly-report?month=${currentMonth}&year=${currentYear}`, {
                headers: { Authorization: token }
            });
            setReport(reportRes.data);

            const ledgerRes = await axios.get(`${API_BASE_URL}/admin/ledger-report?period=weekly`, {
                headers: { Authorization: token }
            });
            ...
            const pendingRes = await axios.get(`${API_BASE_URL}/pending-orders`);
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault();
        const dataToSend = { brand: formData.brand, item_name: formData.item_name, price: Number(formData.price), stock: Number(formData.stock) };
        try {
            if (editingId) {
                await axios.put(`${API_BASE_URL}/update-product/${editingId}`, { price: dataToSend.price, stock: dataToSend.stock });
    ...
    if (!report) return <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh'}}><h2 className="gradient-title">Loading System...</h2></div>;
        ...
        <div className="animate-fade-in dashboard-container page-content" style={{ backgroundColor: '#f8fafc', minHeight: '100vh', paddingBottom: '100px' }}>
            ...
            <div className="bento-grid" style={{ marginBottom: '24px' }}>
                ...
                <div className="bento-item bento-col-1 stagger-1" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                            ...
                            <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#0F172A' }}>Rs. {report.monthly_balance}</div>
                        </div>
                    ...
                    <div style={{ marginTop: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ color: '#059669', background: '#D1FAE5', padding: '4px 8px', borderRadius: '6px', fontWeight: '700', fontSize: '0.75rem' }}>+12%</span>
                        <span style={{ color: '#94A3B8', fontSize: '0.8rem', fontWeight: '500' }}>from last month</span>
                    </div>
                </div>
                ...
                <div className="bento-item bento-col-1 stagger-2" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                            ...
                            <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#0F172A' }}>Rs. 45,000</div>
                        </div>
                    ...
                    <div style={{ marginTop: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ color: '#059669', background: '#D1FAE5', padding: '4px 8px', borderRadius: '6px', fontWeight: '700', fontSize: '0.75rem' }}>+8.5%</span>
                        <span style={{ color: '#94A3B8', fontSize: '0.8rem', fontWeight: '500' }}>from last month</span>
                    </div>
                </div>
                ...
                <div className="bento-item bento-col-1 stagger-3" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <div style={{ marginTop: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ color: '#DC2626', background: '#FEE2E2', padding: '4px 8px', borderRadius: '6px', fontWeight: '700', fontSize: '0.75rem' }}>-2%</span>
                        <span style={{ color: '#94A3B8', fontSize: '0.8rem', fontWeight: '500' }}>from last month</span>
                    </div>
                </div>
                ...
                <div className="bento-item bento-col-1 stagger-4" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                            <h3 style={{ fontSize: '0.85rem', textTransform: 'uppercase', color: '#64748B', letterSpacing: '0.05em', marginBottom: '8px', fontWeight: '700' }}>Total Customers</h3>
                            <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#0F172A' }}>1,240</div>
                        </div>
                    ...
                    <div style={{ marginTop: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ color: '#059669', background: '#D1FAE5', padding: '4px 8px', borderRadius: '6px', fontWeight: '700', fontSize: '0.75rem' }}>+5%</span>
                        <span style={{ color: '#94A3B8', fontSize: '0.8rem', fontWeight: '500' }}>from last month</span>
                    </div>
                </div>
                ...
                <div className="bento-item bento-col-4 stagger-5">
                    ...
                    <form onSubmit={handleFormSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        <div>
                            ...
                            <input className="form-input-modern" type="text" value={formData.brand} onChange={e => setFormData({ ...formData, brand: e.target.value })} required disabled={editingId} placeholder="e.g. Nike" />
                        </div>
                        <div>
                            <label style={{fontWeight: '700', fontSize: '0.85rem', color: '#475569', marginBottom: '5px', display: 'block'}}>Item Name</label>
                            <input className="form-input-modern" type="text" value={formData.item_name} onChange={e => setFormData({ ...formData, item_name: e.target.value })} required disabled={editingId} placeholder="e.g. Air Jordan" />
                        </div>
